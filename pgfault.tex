...
Before proceeding, please read the function {\tt sys\_sbrk()} (see
\texttt{kernel/sysproc.c}, \texttt{sys\_sbrk}) and the page-fault handler
\lstinline{vmfault} (see \texttt{kernel/vm.c}, \texttt{vmfault}).
Also look at the user-trap handler in the interrupt manager, which detects
page faults (see \texttt{kernel/interrupt\_manager.h}, \texttt{user\_interrupt}).
...
(see \texttt{kernel/sysproc.c}, \texttt{sys\_sbrk}).
...
call allocates and maps physical memory immediately, rather than waiting
for a page fault. In classic xv6, this work is done by \lstinline{growproc}
and \lstinline{uvmalloc}. In xv6++, the same responsibilities are split
between the process and the virtual memory manager (see
...
If the application allocates memory lazily, \lstinline{sys_sbrk}
just increments the process's recorded size and returns the old size; it does
...
and calls the page-fault handler \lstinline{vmfault}
(see \texttt{kernel/vm.c}, \texttt{vmfault})
...
allocates a page of physical memory, zeros the allocated page, and adds a PTE
to the user page table.
xv6++ sets the \lstinline{PTE_W}, \lstinline{PTE_R},
\lstinline{PTE_U}, and \lstinline{PTE_V} flags in the PTE for the
new page. Then, the user-trap handler resumes the
...
Since some pages may never have
been used by the process and thus never have been allocated by
\lstinline{vmfault}, the unmap code skips PTEs without the \lstinline{PTE_V}
flag. If a PTE mapping is valid, the kernel frees the physical memory
it refers to.
