...
When \lstinline{kfork} (see \texttt{kernel/proc.c})
...
\indexcode{main}
calls
\indexcode{kvminit} (see \texttt{kernel/vm.c})
to create the kernel's page table using
\indexcode{kvmmake} (see \texttt{kernel/vm.c}).
...
\indexcode{proc_mapstacks} (see \texttt{kernel/proc.c})
...
\indexcode{kvmmap} (see \texttt{kernel/vm.c})
calls
\indexcode{mappages} (see \texttt{kernel/vm.c}),
which
...
and
\lstinline{PTE_V}
to mark the PTE as valid (see \texttt{kernel/vm.c}).

\indexcode{walk} (see \texttt{kernel/vm.c})
...
final page (see \texttt{kernel/vm.c}).
...
next-level-down page table from a PTE (see \texttt{kernel/vm.c}),
...
\indexcode{main}
calls
\indexcode{kvminithart} (see \texttt{kernel/vm.c})
...
The free list is
protected by a spin lock (see \texttt{kernel/kalloc.c}).
...
calls
\indexcode{kinit}
to initialize the allocator (see \texttt{kernel/kalloc.c}).
\lstinline{kinit}
...
The function
\lstinline{kfree} (see \texttt{kernel/kalloc.c})
...
\texttt{MAXVA}
({\tt 0x4000000000})
(see \texttt{kernel/riscv.h}), though in practice
...
\texttt{kernel/elf.h}.  An ELF binary consists of an ELF header,
\indexcode{struct elfhdr} (see \texttt{kernel/elf.h}),
followed by a sequence of program section headers,
\lstinline{struct proghdr} (see \texttt{kernel/elf.h}).  Each
