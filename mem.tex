...
Before proceeding, skim \texttt{kernel/virtual\_memory.h} and its
corresponding \texttt{.cpp} implementation.
...
\item Complete the \texttt{vmprint} system call by making
\texttt{virtual\_memory::vmprint()} walk the calling process's page table
and print each valid leaf mapping. For each mapped page, print the
virtual address and the physical address from the PTE. The provided
\texttt{vmprinttest} program grows its heap and calls \texttt{vmprint};
your output should include the low virtual addresses, the newly allocated
heap pages, and enough mappings to show the structure of the process's
address space.
