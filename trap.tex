...
kernel C++ code, a C++ method that decides what to do with the trap,
...
and the trap-handling methods in the interrupt manager
(see \texttt{kernel/interrupt\_manager.h}, \texttt{user\_interrupt})
and (see \texttt{kernel/interrupt\_manager.h}, \texttt{prepare\_return}).
...
\texttt{kernel/riscv.h} contains definitions that xv6++ uses. Here's
...
then the kernel's user-trap handler
(see \texttt{kernel/interrupt\_manager.h}, \texttt{user\_interrupt});
and when the kernel is ready to return,
the user-trap handler returns to
{\tt userret}
(see \texttt{kernel/trampoline.S}, \texttt{userret})
...
This page contains {\tt uservec}, the trap handling code
...
(see \texttt{kernel/process.h}, \texttt{struct trapframe}). Because {\tt satp} still
...
Each process has a pointer to its trapframe
(see \texttt{kernel/process.h}, \texttt{get\_trapframe}).
...
kernel stack, the current CPU's hartid, the address of the kernel's user-trap handler,
and the address of the kernel page table. {\tt uservec}
retrieves these values, switches {\tt satp} to the kernel page table,
and jumps to the C++ user-trap handler.

The job of the user-trap handler is to determine
...
If the trap is a system call, it invokes the system call dispatcher;
if a device interrupt, it handles the device interrupt;
if a page fault, it handles the fault;
...
The handler checks if the process has been
...
(see \texttt{kernel/interrupt\_manager.h}, \texttt{prepare\_return}).
This method sets up the RISC-V control registers to prepare for a
...
{\tt uservec},
the user-trap handler, and then the system call dispatcher to execute.

At this point, please read {\tt kernel/syscall.cpp},
{\tt sys\_write()} in {\tt kernel/sysfile.cpp},
and the virtual-memory copy routines in the virtual memory manager
(see \texttt{kernel/virtual\_memory.h}, \texttt{copyout}),
(see \texttt{kernel/virtual\_memory.h}, \texttt{copyin}), and
(see \texttt{kernel/virtual\_memory.h}, \texttt{copyinstr}).

\indexcode{syscall}
(see \texttt{kernel/syscall.cpp}, \texttt{syscall})
retrieves the system call number from the saved
\texttt{a7} in the trapframe
and uses it to index into {\tt syscalls}
(see \texttt{kernel/syscall.cpp}, \texttt{syscalls}).
For our example,
\texttt{a7}
contains
\indexcode{SYS_write}
(see \texttt{kernel/syscall.h}, \texttt{SYS\_write}),
...
from the trap frame
as an integer, pointer, or string. File-system calls also use the
\lstinline{argfd} helper in \texttt{kernel/sysfile.cpp} to turn a file
descriptor argument into a file object.
...
{\tt fetchstr} is an example (see \texttt{kernel/syscall.cpp}, \texttt{fetchstr}).
...
\indexcode{copyinstr}
(see \texttt{kernel/virtual\_memory.h}, \texttt{copyinstr}) copies up to \lstinline{max} bytes to
...
{\tt walkaddr}
(see \texttt{kernel/virtual\_memory.h}, \texttt{walkaddr})
...
and the kernel-trap handler in the interrupt manager
(see \texttt{kernel/interrupt\_manager.h}, \texttt{kernel\_interrupt}).

Xv6++ handles traps from kernel code in a different way
than traps from user code.
When entering the kernel, the user-trap handler points {\tt stvec}
to the assembly code at {\tt kernelvec}
(see \texttt{kernel/kernelvec.S}, \texttt{kernelvec}).
...
{\tt kernelvec} jumps to the kernel-trap handler after saving registers
(see \texttt{kernel/interrupt\_manager.h}, \texttt{kernel\_interrupt}).
The kernel-trap handler is prepared for just one type of trap:
device interrupts. It uses the interrupt manager's device-interrupt routine
...
If the kernel-trap handler was called due to a timer interrupt, and a
process's kernel thread is running (as opposed to a scheduler thread),
it will yield the CPU to give other threads a chance to
run. At some point one of those threads will yield, and let our thread
and its kernel-trap handler resume again.
Chapter~\ref{CH:SCHED} explains what happens when yielding.

When the kernel-trap handler's work is done, it needs to return to whatever
code was interrupted by the trap. Because a yield may have
disturbed {\tt sepc} and the previous mode in {\tt sstatus},
the handler saves them when it starts. It now restores those
...
enters the kernel from user space; you can see this in the user-trap handler
(see \texttt{kernel/interrupt\_manager.h}, \texttt{user\_interrupt}).
...
to take a trap, and the user-trap handler doesn't enable them again until
