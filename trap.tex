...
then {\tt usertrap}
(see \texttt{kernel/trap.c}, \texttt{usertrap});
...
(see \texttt{kernel/proc.h}, \texttt{struct trapframe}). Because {\tt satp} still
...
(see \texttt{kernel/trap.c}, \texttt{prepare\_return}).
...
\indexcode{copyinstr}
(see \texttt{kernel/vm.c}, \texttt{copyinstr}) copies up to \lstinline{max} bytes to
...
{\tt walkaddr}
(see \texttt{kernel/vm.c}, \texttt{walkaddr})
...
(see \texttt{kernel/trap.c}, \texttt{kerneltrap}) after saving registers.
{\tt kerneltrap} is prepared for just one type of trap:
device interrupts. It calls
{\tt devintr}
(see \texttt{kernel/trap.c}, \texttt{devintr})
to handle them.
...
(see \texttt{kernel/kernelvec.S}, \texttt{kerneltrap}).
...
(see \texttt{kernel/trap.c}, \texttt{kernelvec}).
