...
Xv6++ is a C++ reimplementation of xv6 intended for teaching.
It aims to preserve xv6's simplicity and clarity while restructuring
the kernel using a small, explicit subset of C++ (classes, methods, and
encapsulation), without exceptions or the C++ standard library.
The goal is not to hide low-level behavior behind abstractions, but to
organize the code so that subsystem boundaries and responsibilities are
clearer.  Throughout this book, the user-visible interface is the same
Unix-like system call interface; the primary differences are in the
kernel's internal organization.
...
The xv6++ shell is implemented in \texttt{user/sh.c}.
A good practice is to read the source code as you work through the chapter,
and consult this text when you want help interpreting what you find.
...
This chapter focuses on xv6++'s \emph{interface}---the system calls and the
user-level conventions that make them useful (for example, the shell's use of
file descriptors for I/O redirection and pipelines).
Most of these system calls are intentionally compatible with xv6 and traditional
Unix, so that the same small set of mechanisms can be combined in many ways.

The \emph{implementation} of these calls inside the kernel is where xv6++ differs
most from xv6.  Xv6++ uses a modest subset of C++ to organize kernel subsystems
into classes with clearer ownership and boundaries (for example, process
management, the buffer cache, and file-system logging).
These internal changes are meant to improve readability and structure, not to
change the user-visible behavior of the system calls in Figure~\ref{fig:api}.
...
\lstinline{main}
(see \texttt{user/sh.c}, \texttt{main}).
...
as the argument.
\lstinline{runcmd}
(see \texttt{user/sh.c}, \texttt{runcmd})
runs the actual command. For
``\lstinline{echo hello}'',
it would call
\lstinline{exec}
(see \texttt{user/sh.c}, \texttt{exec.ecmd}).
If
\lstinline{exec}
...
(see \texttt{user/sh.c}, \texttt{case REDIR}).
...
(see \texttt{user/sh.c}, \texttt{case PIPE}).
...
\item Write a user-level \texttt{sleep} program that pauses
execution for a number of timer ticks given on the command line.
The program should validate that an argument was provided, convert
it to an integer, call the existing \texttt{sleep} system call, and
then exit.

\item Write a user-level \texttt{pingpong} program that creates a
child process and uses pipes to exchange one byte between parent and
child. The child should print that it received \texttt{ping}, reply
through a second pipe, and the parent should print that it received
\texttt{pong} before waiting for the child to exit.
