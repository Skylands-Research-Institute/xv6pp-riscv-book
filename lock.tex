...
The page allocator maintains a linked list of free pages
(see \texttt{kernel/page\_allocator.h}):
allocating a page pops from the head of the free list, and freeing a
page pushes a page onto the list.
...
Please read \texttt{kernel/spin\_lock.h}, \texttt{class spin\_lock},
and \texttt{kernel/sleep\_lock.h}, \texttt{class sleep\_lock}.
...
\midrule
\texttt{kernel.cache.lock} & Protects allocation and list management of block buffer cache entries \\
...
\item Modify the page allocator to reduce lock contention by giving each
CPU its own free list and lock. Allocation and freeing should normally
use the current CPU's list, so simultaneous allocations on different CPUs
can proceed without sharing one global allocator lock. If a CPU's free
list is empty, it should safely steal pages from another CPU's list.
Preserve allocator correctness under concurrency and verify the change by
running \texttt{usertests -q}.
