...
In xv6++ the disk and its driver are available as
\lstinline{kernel.disk}
and
\lstinline{kernel.disk_driver}.
...
data stored in any particular block.
In xv6++ this is the
\lstinline{buffer_cache}
class.
...
In xv6++ inode management is implemented by
\lstinline{file_system}.
...
and resolves them by walking the pathname one component at a time.
...
In xv6++ the buffer cache is implemented by
\lstinline{class buffer_cache}.
...
Xv6++ also provides an optional RAII helper,
\lstinline{buffer_guard},
...
The buffer cache searches from the tail of its list for an unreferenced buffer
to recycle. Newly read buffers move to the front of the list; released buffers
with no remaining references move to the back.
...
and maintains its cache list using an intrusive doubly-linked list
...
In xv6++ the virtio driver methods operate on
\lstinline{buf*}
objects and are invoked through the disk abstraction.
...
If the release drops the buffer's reference count to zero,
\lstinline{relse()} moves it to the tail of the cache list, where it is eligible
for reuse by a later cache miss.
...
In xv6++ the logging layer is implemented by
\lstinline{class file_system_log}
and is available as
\lstinline{kernel.log}.
...
\lstinline{log_op_guard} calls \lstinline{begin_op()} in its constructor and
\lstinline{end_op()} in its destructor.
...
The log conservatively assumes that each file-system system call
might write up to
...
\lstinline{balloc} returns zero if no free block is available; callers such as
\lstinline{bmap()} propagate that allocation failure to higher layers.
...
If no free inode exists, \lstinline{ialloc()} returns \lstinline{nullptr}.
...
If the inode has not yet been read from disk, it also reads
the on-disk inode into the in-memory inode structure.
...
Truncation frees all of a file’s blocks and resets its size to zero.
\lstinline{itrunc()}
...
All open file objects are allocated from a global table managed by
\lstinline{file_manager}.
It allocates and tracks both
\lstinline{file}
...
using \lstinline{log_op_guard} to enter and leave the log transaction.
...
\lstinline{open} also handles \lstinline{O_TRUNC} for regular files and follows symbolic links.
\lstinline{sys_symlink} creates symbolic-link inodes whose contents store the target path.
...
Xv6++ uses a simple list-based eviction policy; many more complex policies exist.
...
\item Add a \lstinline{seek(fd, offset, whence)} system call to reposition
an open file using \lstinline{SEEK_SET}, \lstinline{SEEK_CUR}, or
\lstinline{SEEK_END}. Allow positions from zero through the file size,
returning \lstinline{0} on success and \lstinline{-1} on failure.
Reject invalid descriptors, pipes, devices, and out-of-range positions,
leaving the offset unchanged on failure. Verify the change by running
\texttt{seektest} and \texttt{usertests -q}.

\item Add symbolic links to xv6++. A symbolic link is a file whose
contents name another path. Add a \lstinline{symlink(target, linkpath)}
system call that creates a new \lstinline{T_SYMLINK} inode at
\lstinline{linkpath} and stores \lstinline{target} as the inode's
contents. The target path need not exist when the link is created; the
system call should fail if \lstinline{linkpath} already exists. Modify
\lstinline{open} so that opening a symbolic link follows the target path,
and follows chains of symbolic links up to a fixed limit to avoid cycles.
Opening a dangling symbolic link should fail. \lstinline{link} and
\lstinline{unlink} should operate on the path named by the caller and
need not follow a final symbolic-link component. Verify the change by
running \texttt{symlinktest} and \texttt{usertests -q}.
