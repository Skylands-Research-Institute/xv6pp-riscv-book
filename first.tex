%   terminology:
%     process: refers to execution in user space, or maybe class process &c
...
RISC-V.  RISC-V is a 64-bit CPU, and xv6++ is written primarily in ``LP64'' C++,
...
\midrule
Processes & process.cpp & Process abstraction. \\
 & process\_scheduler.cpp & Scheduling policy. \\
 & process\_manager.cpp & Process creation and management. \\
 & swtch.S & Thread switching. \\
 & sysproc.cpp & Process-related system calls. \\
\midrule
Traps & kernelvec.S & Handle traps from kernel code. \\
 & trampoline.S & Handle traps from user code. \\
 & interrupt\_manager.cpp & Trap and interrupt handling. \\
 & syscall.cpp & System call dispatch. \\
\midrule
Memory & virtual\_memory.cpp & Page tables and address spaces. \\
 & page\_allocator.cpp & Physical page allocator. \\
\midrule
Devices & console\_interface.cpp & Console device. \\
 & uart16550\_driver.cpp & Serial driver. \\
 & virtio\_driver.cpp & Disk device driver. \\
 & printf.cpp & Formatted console output. \\
\midrule
File system & buffer\_cache.cpp & Disk block cache. \\
 & file\_system.cpp & File system. \\
 & file\_system\_log.cpp & Logging and crash recovery. \\
 & file.cpp & Open file abstraction. \\
 & pipe.cpp & Pipe abstraction. \\
 & sysfile.cpp & File-related system calls. \\
\midrule
Locks \& utilities & spin\_lock.cpp & Spin locks. \\
 & sleep\_lock.cpp & Sleep locks. \\
 & string.c & C string library. \\
\end{tabular}
\caption{Xv6++ kernel source files (representative modules).}
\label{fig:source}
\end{figure}

The xv6++ kernel source is in the {\tt kernel/} sub-directory.
Figure~\ref{fig:source} lists representative source files,
organized by major areas of kernel responsibility: booting,
process management, trap handling, virtual memory,
device drivers, and the file system.

In xv6++, these subsystems are gathered into a single top-level
object of type \lstinline{xv6pp}
(see \texttt{kernel/xv6pp.h}).
A single global reference, \lstinline{kernel}, provides access to
this object throughout the system.
Rather than relying on global variables scattered across the kernel,
xv6++ organizes the operating system as a collection of cooperating
subsystem objects—process management, memory management,
device drivers, scheduling, interrupt handling, and the file system—
held together by this top-level kernel object.
...
0x3fffffffff, which is \lstinline{MAXVA} (see \texttt{kernel/riscv.h}).
...
\indexcode{class process}
(see \texttt{kernel/process.h}).
...
At this point, please read \texttt{kernel/process.h}, which defines {\tt
  class process}. The xv6++ code is more important for you to understand
...
Please read \texttt{kernel/entry.S}, \texttt{kernel/start.c},
\texttt{kernel/main.cpp}, and \texttt{user/init.c}.
...
\lstinline{userinit}
(see \texttt{kernel/process\_manager.cpp}).
All newly created processes start executing in the kernel in
\lstinline{forkret}
(see \texttt{kernel/process\_manager.cpp}).
As a special case for the first process, {\tt forkret}
initializes the file system and then calls {\tt exec}
to load the user program {\tt /init}.
...
\item Add a \texttt{getppid} system call to xv6++ that returns the
process ID of the calling process's parent. Add the process accessor
needed to retrieve the parent PID, then connect the system call through
the kernel handler, dispatch table, system call number, user-space
prototype, and user stub. The provided \texttt{getppidtest} program
forks and checks that the child's \texttt{getppid} result matches the
parent's \texttt{getpid} result.
